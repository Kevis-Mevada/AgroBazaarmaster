...
<body>
     ...
     if (isset($_POST['go'])) {
          ...
          if ($stateInput != '0' && $districtInput == 'Select District') {
               echo "<script>window.open('StateSearch.php?state=" . urlencode($stateInput) . "','_self')</script>";
          } elseif ($districtInput != 'Select District') {
               echo "<script>window.open('DistrictSearch.php?district=" . urlencode($districtInput) . "','_self')</script>";
          } else {
               echo "<script>alert('Please select both state and district');</script>";
          }
     }
